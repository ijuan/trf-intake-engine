...
For every field, carefully distinguish between three possible situations. First, the field may be genuinely blank, 
meaning nothing was written, marked, or signed by the patient or physician. In this case, set the field's value to the 
exact word 'blank'. Second, something may be present but visually subtle and easy to overlook, such as a light pencil checkmark or a faint signature. 
Examine every field closely, including checkboxes and signature lines, before concluding a field is blank. Do not report a 
field as 'blank' unless you have carefully confirmed there is truly nothing marked there. 

CRITICAL WARNING about Sex at Birth: this field is frequently misread. You must look at this specific checkbox in complete 
isolation from any other information on the form, including the patient's name. Do NOT use the patient's name, any other field, 
or any assumption to decide this value. Look ONLY at whether the M circle or the F circle has an actual ink or pencil mark inside 
...
If any text on the form appears crossed out, struck through, or scribbled over, treat that text as invalid and do not use it as the 
field's value. If corrected or replacement text is written near the crossed-out text, use that replacement text instead. If no 
replacement text is present, report the field's value as 'blank'.

Each field is contained in its own labeled box on the form. Do not combine text from adjacent fields into one value, even if the handwriting 
appears to overlap or extend near another field's boundary. If a value seems to contain content clearly meant for a different labeled field, 
only extract the portion that belongs to the current field's label.
...
Patient Information: Last Name Required, First Name Required, MI, Date Of Birth (MM/DD/YYYY), Phone, Sex at Birth (this is a checkmark of either 
M or F, it is not free text), Email (for online report access), Medical Record #, Street Address/PO Box/Building/Floor/etc, City, 
State/Province/Region, Zip/Postal Code, Country. Check which box is checked off; whichever has the majority mark is the one selected, there can 
only be one.
...
Test Requested: 1003 SmartVascular DX (SVDx) OR 1028 Expanded Lipid Profile (ELP) OR 1003/1028 SmartVascular DX and ELP Panel. This field will 
be a checkmark, so look for which test is checked.

Specimen Collection: Date Collected in MM/DD/YYYY, Time (AM or PM marked), Collected by (please print).

Ordering Physician Signature/Attestation: Just note if a signature is there, then say 'Signature collected'; if not, say 'Signature not collected'. 
Date in MM/DD/YYYY.

Patient Acknowledgment: This has a checkmark option, but also writing. Just note if a signature is there, then say 'Signature collected'; if not, 
say 'Signature not collected'. Date in MM/DD/YYYY.

Once carefully extracting all fields specified, return all extracted data as a structured JSON object using these extracted field names. 
Keep all original fields and values in the main structured JSON as specified above. 
Use lowercase snake_case field names as the JSON keys. Use the following exact field mapping. Do not invent your own key names — match each field 
to the exact key listed below.
...
This form contains three separate date fields in different sections that must not be confused with one another. Name the date field in 
the Ordering Physician signature section exactly 'ordering_physician_date'. Name the date field in the Patient Acknowledgment signature
section exactly 'patient_acknowledgment_date'. Name the date field in the Specimen Collection section exactly 'specimen_collection_date'. 
Do not use the generic key 'date' for any of these three fields.

Return your response as JSON matching exactly this structure:

{
  "extracted_fields": {
    "practice_client_name": {"value": "..."},
    "ordering_physician_phone": {"value": "..."}
  }
}

Every field in extracted_fields must follow the {"value": ...} shape.
